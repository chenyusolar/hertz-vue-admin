...
func (e *CustomerApi) CreateExaCustomer(ctx context.Context, c *app.RequestContext) {
	...
	customer.SysUserID = utils.GetUserID(ctx, c)
	customer.SysUserAuthorityID = utils.GetUserAuthorityId(ctx, c)
...
func (e *CustomerApi) GetExaCustomerList(ctx context.Context, c *app.RequestContext) {
	...
	customerList, total, err := customerService.GetCustomerInfoList(utils.GetUserAuthorityId(ctx, c), pageInfo)
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
