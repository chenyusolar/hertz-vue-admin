...
func (b *FileUploadAndDownloadApi) BreakpointContinue(ctx context.Context, c *app.RequestContext) {
	fileMd5 := string(c.PostForm("fileMd5"))
	fileName := string(c.PostForm("fileName"))
	chunkMd5 := string(c.PostForm("chunkMd5"))
	chunkNumber, _ := strconv.Atoi(string(c.PostForm("chunkNumber")))
	chunkTotal, _ := strconv.Atoi(string(c.PostForm("chunkTotal")))
...
func (b *FileUploadAndDownloadApi) FindFile(ctx context.Context, c *app.RequestContext) {
	fileMd5 := string(c.Query("fileMd5"))
	fileName := string(c.Query("fileName"))
	chunkTotal, _ := strconv.Atoi(string(c.Query("chunkTotal")))
	file, err := fileUploadAndDownloadService.FindOrCreateFile(fileMd5, fileName, chunkTotal)
	if err != nil {
