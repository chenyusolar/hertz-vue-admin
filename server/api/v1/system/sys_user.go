...
// TokenNext 登录以后签发jwt
func (b *BaseApi) TokenNext(c *app.RequestContext, user system.SysUser) {
...
func (b *BaseApi) SetUserAuthority(ctx context.Context, c *app.RequestContext) {
	...
	claims := utils.GetUserInfo(ctx, c)
...
func (b *BaseApi) SetUserAuthorities(ctx context.Context, c *app.RequestContext) {
	...
	authorityID := utils.GetUserAuthorityId(ctx, c)
...
func (b *BaseApi) SetSelfSetting(ctx context.Context, c *app.RequestContext) {
	...
	err = userService.SetSelfSetting(req, utils.GetUserID(ctx, c))
	if err != nil {
		global.GVA_LOG.Error("设置失败!", zap.Error(err))
		response.FailWithMessage("设置失败", c)
		return
...
func (b *BaseApi) GetUserInfo(ctx context.Context, c *app.RequestContext) {
	...
	response.OkWithDetailed(map[string]interface{}{"userInfo": ReqUser}, "获取成功", c)
}
